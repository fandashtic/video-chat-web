...
try {
  randomNumber =
    Number.parseInt(new Date().getTime(), 10) +
    Math.floor(Math.random() * 1000);
} catch (error) {
  randomNumber = (new Date().getTime(), 10) + Math.floor(Math.random() * 1000);
}
let _testChannel = String(randomNumber);
...
const uiInit = () => {
  ...
  return new Promise((resolve, reject) => {
    ...
    axios
      .get(
        "https://us-central1-videocall-3e257.cloudfunctions.net/generateRtcToken?channelName=" +
          info.channel
      )
      .then(result => {
        var agoraIo = result.data;
        if (agoraIo !== null && agoraIo !== undefined) {
          Cookies.set("APP_ID", agoraIo.appId);
          Cookies.set("Token", agoraIo.token);
        }
        console.log(JSON.stringify(agoraIo));
      });
    ...
    if (AgoraRTC.checkSystemRequirements()) {
      $("#compatibility").html(
        "Chrome, Firefox, Safari, Opera, QQ Browser, 360 Secure. Others are not fully supported and some functions may be lost"
      );
    } else {
      $("#compatibility").html(
